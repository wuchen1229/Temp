...
f0=-arfa2/(3*arfa1)-(C1*(x0*dB+Bmin)^2/((z0*dz+zmin)^2*(x0*dB+Bmin-y0*dCom-Com_min)^2)+C2*(x0*dB+Bmin)/(x0*dB+Bmin-y0*dCom-Com_min)+C3*(u0*dCin+Cin_min)/((z0*dz+zmin)*(x0*dB+Bmin-y0*dCom-Com_min))+C4)^(1/2);
%鍋忓
dfx=diff(f,x);
dfy=diff(f,y);
dfz=diff(f,z);
...
tableB = array2table(Result_r);
saveName = 'Result_r.xlsx';
writetable(tableB, saveName, "Sheet", "折现率");